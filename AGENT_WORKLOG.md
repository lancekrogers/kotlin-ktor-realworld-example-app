...
Every changed test was recorded failing against deliberately broken behaviour before being recorded passing against the real code, with `--rerun-tasks --no-build-cache` — a reverted file returns the build to a cached state, and Gradle will otherwise report success having executed nothing. Suite: **95 running, 0 failing, 16 skipped**.
