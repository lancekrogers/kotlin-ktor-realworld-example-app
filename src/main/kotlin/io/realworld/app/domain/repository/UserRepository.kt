...
class UserRepository {
    ...
    fun findIsFollowUser(email: String, followedUserId: Long): Boolean {
        return transaction {
            val follower = findByEmail(email) ?: return@transaction false
            Follows.select {
                (Follows.user eq followedUserId) and (Follows.follower eq follower.id!!)
            }.count() > 0
        }
    }
